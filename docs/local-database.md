...
`pnpm db:init` 会执行 `supabase/migrations/20260730074758_create_experiments_catalog.sql`，然后执行 `supabase/seed.sql` 写入三个免费实验目录记录。它只用于新建的空数据库；迁移已执行后不要重复运行。已初始化的数据库只需单独重放幂等的 `supabase/seed.sql`。
