...
模板扩展遵循 [实验模板路线](experiment-roadmap.md)：Free Starter 固定为四个实验；付费内容已完成 DC Circuits、Waves、Density/Buoyancy、Momentum/Collisions、Refraction/Total Internal Reflection、Levers/Balance、Sound、Electrical Power/Energy、Electromagnets 与 Lenses/Image Formation 十个模板。下一主题继续由教师重复使用、模板请求和付费数据决定，不预先建设通用编辑器。
